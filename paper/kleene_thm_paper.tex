...
\label{ch:finite_automata}
	...
	\begin{remark}
		...
		A NDFA is considered to be in an accepting state when any of its accepting states are active.
...
\label{ch:kleenes_theorem}
	...
	\label{proof:ndfa_dfa_equivalent}
		Since every DFA is a NDFA, every language represented by a DFA is trivially represented by a NDFA.
		We need only prove that any language represented by a NDFA can be represented by a DFA.

		Let $M = (Q, \Sigma, s, \delta, F)$ be a NDFA.

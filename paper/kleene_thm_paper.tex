...
\label{ch:finite_automata}
	...
	\begin{example}[Finite Automata Construction]
		...
		In this example, we will give a step-by-step construction of a FA to recognize the language of binary numbers divisible by 3.
		...
		We begin by defining transitions from $q_0$.
		Suppose that $\miii$ is in its initial state: $q_0$.
		If the first input symbol is 0, then the value of the input does not change and the machine should not change state.
		So we define $\delta(q_0, 0) = q_0$.
		But if the first symbol is 1, the value of the input is $1_b = 1 \mod 3$, so the next state should be $q_1$, and we define $\delta(q_0, 1) = q_1$.
		...
		Clearly, we don't need to add any more transitions from $q_0$, so we move on to $q_1$.
		Imagine that the input so far is 1, so that $\miii$ will be in state $q_1$.
		If the next input symbol is 0, we have $10_b = 2 = 2 \mod 3$, and the next state should be $q_2$.
		So we define $\delta(q_1, 0) = q_2$.
		But if the next symbol is 1, we have $11_b = 3 = 0 \mod 3$. 
		...
		If the next symbol is 0, we have $100_b = 4 = 1 \mod 3$, so we define $\delta(q_2, 0) = q_1$.
		But if the next symbol is 1, we have $101_b = 5 = 2 \mod 3$, so we set $\delta(q_2, 1) = q_2$.
	...
	\begin{definition}[Deterministic and Nondeterministic Finite Automata]
		All finite automata as we have described them fall into the class of \textit{nondeterministic} finite automata (NDFAs).
		Any FA with zero or more transitions from each state on a given symbol is a NDFA.
	...
	\begin{remark}
		...
		It is important to note that this is not the case for NDFAs.
		Because a NDFA may have any number of transitions from each state on a single symbol, any number of its states may be active at any given time.

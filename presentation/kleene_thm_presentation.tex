...
\end{frame}
\begin{frame}
	\frametitle{Building Regular Expressions for Paths}
...
\begin{frame}
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=3cm]
			...
			\path[->] (qi) edge   [wavy, swap]                                                      node {$R_k(i, k)$}     (qk);
			\path[->] (qk) edge   [less wavy, loop, out = 315, in = 235, looseness = 5, pos = 0.17] node {$(R_k(k, k))^*$} (qk);
			\path[->] (qk) edge   [wavy, swap]                                                      node {$R_k(k, j)$}     (qj);
		\end{tikzpicture}
	\end{figure}
\end{frame}
...
\begin{frame}
	...
	\begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=2cm]
		...
		\path[->] (qx)  edge [loop right, out = 350, in = 300, distance = 0.7 cm] node {$a, b$} (qx);

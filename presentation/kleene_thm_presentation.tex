...
\note[itemize]{
	...
	\item We already used the notation: the Kleene closure of an alphabet is the language of all possible strings over that alphabet
...
\end{frame}
\note[itemize]{
	\item Two ways of representing regular languages.
...
\begin{frame}
	...
	For $w = abc \in \Sigma^*$:
	\begin{align*}
		\Delta(q_1, w) &= \delta(\Delta(q_1, ab), c) \\
...
\begin{frame}
	...
	\begin{itemize}
		...
		\pause
		\item[] $Q' = 2^Q$
		\item[] $\{ q_1, q_2, \dots, q_n \} \subseteq Q \quad \implies \quad \{ q_1, q_2, \dots, q_n \} \in Q'$.
		\pause
		\item[] $s' = \{ s \}$
		\pause
...
\note[itemize]{
	...
	\item[\click] This construction works because the active state in the DFA always corresponds exactly to the set of active states in the original NDFA
...
\begin{frame}
	...
	Number these states $q_1, q_2, \dots, q_n$.

	\[
		R_0(i, j) = \{ a \in \Sigma^* \mid \delta(q_i, a) = q_j \}
	\]
...
\begin{frame}
	\frametitle{Example of that Last Bit}
	Remember this?
	\begin{figure}[H]
		\centering
